...
local function LoadModule(moduleName)
    ...
    if success and module and type(module) == "table" then
        print("[ASTRA] Модуль " .. moduleName .. " успешно загружен!")
        return module
    else
        print("[ASTRA] Модуль " .. moduleName .. " не найден или не является таблицей.")
        return nil
    end
end)

-- Проверяем, какая игра
